...
class Testsolver(BaseSolver):
    ...
    def check(self):
        ...
        if self.cuda:
            ...
            self.model_path = self.cfg['test']['model']
    ...
    def test(self):
        ...
        for batch in self.data_loader:
            ...
            self.save_img(ms_image.cpu().data, name[0][0:-4] + '_gt.mat', mode=None)
            self.save_img(prediction.cpu().data, name[0][0:-4] + '.mat', mode=None)
    def eval(self):
        self.model.eval()
        avg_time= []
        for batch in self.data_loader:
            ...
            print("===> Processing: %s || Timer: %.4f sec." % (name[0], (t1 - t0)))
            avg_time.append(t1 - t0)
            self.save_img(bms_image.cpu().data, name[0][0:-4]+'_bic.mat', mode=None)
            self.save_img(prediction.cpu().data, name[0][0:-4]+'.mat', mode=None)
    ...
    def save_img(self, img, img_name, mode):
        ...
        save_img = img.squeeze().cpu().numpy().transpose(1, 2, 0)
        
        key = 'result'
        if '_gt' in img_name:
            key = 'gt'
        elif '_bic' in img_name:
            key = 'bic'

        scipy.io.savemat(save_fn, {key: save_img})
